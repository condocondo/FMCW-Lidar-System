...
for i in range(5):
    sum_noise_floor_itteration = 0
    noise_floor_itteration = np.zeros(100)
    itteration_index = np.zeros(100)
    for l in range(i) :
        AF = get_AF_discretized_phase_error(N_antennas_per_block=N_antennas_per_block, N_block=N_block, array_pitch=array_pitch, delay_length=delay_length, n_eff=neff, wavelength=wavelength_min, angles=angles, phase_error_order = phase_error_order)
        ...
        sum_noise_floor_itteration += noise_floor
        l+=1
    
    noise_floor_itteration[i] = sum_noise_floor_itteration / l
    itteration_index[i] = i
...
plt.plot(itteration_index, noise_floor_itteration)

# Limit x and y axes
#plt.xlim(1, 20)  # Set limit for x axis
