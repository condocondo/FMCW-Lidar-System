...
def get_array_factor_1D_phase_error(n_eff, wavelength, delay_length, angles, positions, amplitudes, delay_factors, phase_error_order): # len(positions) = len(amplitudes) = len(delay_factors) = N
    ...
    for i in range(len(positions)):
        ...
        phase = np.random.rand()*np.pi/phase_error_order
...
for i in range(10):
    ...
    AF_noisy= get_AF_discretized_phase_error(N_antennas_per_block=N_antennas_per_block, N_block=N_block, array_pitch=array_pitch, delay_length=delay_length, n_eff=neff, wavelength=wavelength_min, angles=angles, phase_error_order = 10)
    AF_noise_free = get_AF_discretized_uniform_array(N_antennas_per_block=N_antennas_per_block, N_block=N_block, array_pitch=array_pitch, delay_length=delay_length, n_eff=neff, wavelength=wavelength_min, angles=angles)
    AF_noisy_dB = np.max(10 * np.log10(np.abs(AF_noisy) ** 2))
    AF_nois_free_dB = np.max(10 * np.log10(np.abs(AF_noise_free) ** 2))
    AF_noisy_normalized_dB = 10 * np.log10(np.abs(AF_noisy) ** 2) - AF_noisy_dB
    AF_nois_free_normalized_dB = 10 * np.log10(np.abs(AF_noise_free) ** 2) - AF_nois_free_dB
    noise = AF_nois_free_normalized_dB - AF_noisy_normalized_dB
    #AF_intensity = np.abs(AF) ** 2
    #AF_normalized = AF_intensity / np.max(AF_intensity)
    plt.plot(angles, noise, label="um, "+ "Array pitch = "+ str(array_pitch)+"um, "+ "N = "+ str(N)+"phase error = pi / "+str(10*i))
