...
def gaussian_amplitudes(n_antennas, amplitude_sigma = 0.2):
    # gaussian with unity power
    n = n_antennas
    sigma = amplitude_sigma*n
    i = np.linspace(-0.5 *(n-1), 0.5 *(n -1), n)
    g = np.exp(-0.5 *(i/sigma)**2)
...
def get_AF_discretized_amp_error(N_antennas_per_block, N_block, array_pitch, delay_length, n_eff, wavelength, angles):
    ...
    amplitude_sigma = 0.2
    amplitudes = gaussian_amplitudes(n_antennas = N, amplitude_sigma = amplitude_sigma)
    #print(str(amplitude))
...
for i in range(1):
    ...
    AF = get_AF_discretized_amp_error(N_antennas_per_block=N_antennas_per_block, N_block=N_block, array_pitch=array_pitch, delay_length=delay_length, n_eff=neff, wavelength=wavelength_min, angles=angles)
    ...
    plt.plot(angles, AF_normalized_dB, label="gaussian distribution of amplitude, attenuation = "+str())
