...
def get_array_factor_1D_generic_snake(n_eff, wavelength, delay_length, angles, positions, amplitudes, delay_factors, coherent_length): # len(positions) = len(amplitudes) = len(delay_factors) = N
    ...
    for i in range(1, len(positions)):
        mean = 0   
        standard_deviation[i] = np.sqrt(standard_deviation[i-1]**2 + SD**2)
...
def get_AF_discretized_uniform_array(N_antennas_per_block, N_block, array_pitch, delay_length, n_eff, wavelength, angles, coherent_length):
    ...
    return get_array_factor_1D_generic_snake(n_eff, wavelength, delay_length, angles, positions, amplitudes, delay_factors, coherent_length)
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(15, 10))
    plt.title("phase_error[i] = phase_error[i-1] + pi * random_number,    phase_error_strength =" +str(phase_error_strength)+",    coherent_length = "+str(coherent_length)+"um")
    plt.xlabel("Antenna Number")
    plt.ylabel("phase_error_amplitude")
    plt.plot(phase_error_index, phase_error)

    plt.figure(figsize=(15, 10))
    plt.title("Accumalative_standard_deviation,    coherent_length = "+str(coherent_length)+"um")
    plt.xlabel("Antenna Number")
